...
// --- MODULE 1: SỐ HÓA HỒ SƠ VÀ QUẢN LÝ VĂN BẢN APIs ---
...
app.post('/api/digitize/upload', upload.single('file'), async (req, res) => {
  try {
    ...
    // Handle text extraction based on mime type
    if (mimetype === 'application/vnd.openxmlformats-officedocument.wordprocessingml.document') {
      // DOCX parsing with mammoth
      const result = await mammoth.extractRawText({ path: filePath });
      extractedText = result.value;
    } else if (mimetype === 'application/pdf' || mimetype.startsWith('image/')) {
      // PDF and Images can be parsed directly by Gemini
    ...
    // Now, analyze with Gemini to extract metadata in structured format
    ...
    if (metaResponse.text) {
      try {
        ...
        console.error('Error parsing Gemini metadata response, fallback used', parseErr);
      }
    }
    ...
    res.status(500).json({ error: error.message || 'Lỗi xử lý số hóa tài liệu' });
  }
});
